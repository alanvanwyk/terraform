...
func (lt *opsworksLayerType) VolumeConfigurations(d *awsResourceData) []*opsworks.VolumeConfiguration {
	...
	for i := 0; i < len(configuredVolumes); i++ {
		...
		result[i] = &opsworks.VolumeConfiguration{
			...
			VolumeType:    aws.String(volumeData["volume_type"].(string)),
		}
...
func (lt *opsworksLayerType) SetVolumeConfigurations(d *awsResourceData, v []*opsworks.VolumeConfiguration) {
	...
	for i := 0; i < len(v); i++ {
		...
		if config.VolumeType != nil {
			data["volume_type"] = *config.VolumeType
		}
	}
